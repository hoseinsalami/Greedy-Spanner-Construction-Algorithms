...
private:
	...
	void minHeapify(unsigned int i, bool test = false) {
...
public:
	...
	void remove(const unsigned int key) {
		...
		unsigned int cur = lookup[key];
		unsigned int parent = parentIndex(lookup[key]);

		if (parent < myCount && values[cur].second < values[parent].second)
	...
	void increaseKey(const unsigned int key, const TValue newValue, bool test = false) {
		double oldVal = values[lookup[key]].second;
		unsigned int ke = lookup[key];
		values[lookup[key]].second = newValue;
		minHeapify(lookup[key], test);
	}
	void extractMin() {
		myCount--;
