...
void Delaunay::Triangulate(std::string& triswitches){
  ...
  /*   box vertex in insertvertex().                                 */
  	tpmesh->infvertex1 = (piyush::vertex) nullptr;
  	tpmesh->infvertex2 = (piyush::vertex) nullptr;
  	tpmesh->infvertex3 = (piyush::vertex) nullptr;
...
void Delaunay::writeoff(std::string& fname){
	...
	pdelclass->writeoff(tpmesh, tpbehavior, pfname, 0, nullptr);
	delete [] pfname;
}
...
Delaunay::vIterator Delaunay::vend(){
	vIterator vit;
	vit.vloop = ((piyush::vertex) nullptr);
	return vit;
}
